    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <form id="descuento" action="{{ route('presupuestos.descuento', ['id' => $results->first()->id]) }}" method="post">
                    @csrf
                    <input type="hidden" name="id" value="{{ $results->first()->id }}">
                    <div id="seccionOcultable">
                        <div class="mb-3">
                            <label for="exampleFormControlInput1" class="form-label">Valor de proyecto</label>
                            <input type="text" name="presupuesto_total" class="form-control custom-input" id="presupuesto_total" placeholder="${{ number_format($results->first()->TOTAL_PROYECTO, 0, ',', '.') }}">
                        </div>
                    </div>
                    <div class="mb-3" id="divDescuento">
                        <label for="exampleFormControlTextarea1" class="form-label">% de descuento</label>
