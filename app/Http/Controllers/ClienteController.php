...
class ClienteController extends Controller
{
    ...
    public function listar_clientes(Request $request)
    {
        ...
        if ($id != 1) {
            $clientes = DB::select('SELECT * FROM clientes WHERE id_user = ? ORDER BY created_at DESC', [$userID]);
        } else {
            $clientes = DB::select('SELECT clientes.*, users.name FROM clientes JOIN users ON clientes.id_user = users.id ORDER BY created_at DESC');
        }
        ...
        return view('clientes.index', compact('clientes', 'departamentos', 'ciudades'));
    }
    ...
    public function crear(Request $request)
    {
        try {
            ...
            $validar_datos = $request->validate([
                'NIC' => 'required|unique:clientes,NIC',
                'id_user' => 'required',
            ...
            return redirect()->back()->withErrors($e->errors())->withInput()->with('duplicateNIC', true);
        }
    }
    ...
    public function actualizar(Request $request, $id)
    {
        ...
        if (isset($datosCliente['consumo_actual'])) {
            $datosCliente['consumo_actual'] = preg_replace('/\D/', '', $datosCliente['consumo_actual']);
        }

        if (isset($datosCliente['tarifa'])) {
            $datosCliente['tarifa'] = preg_replace('/\D/', '', $datosCliente['tarifa']);
        }

        Cliente::where('NIC', '=', $id)->update($datosCliente);
        return redirect()->route('clientes.listar')->with('rgcmessage', '¡El Cliente se a actualizado correctamente!...');
    }
