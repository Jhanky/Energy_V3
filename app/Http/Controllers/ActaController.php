...
class ActaController extends Controller
{
    ...
    public function guardarOrden(Request $request)
    {
        ...
        // Redirigir con mensaje de éxito
        return redirect()->back()->with('success', 'Orden de servicio creada con éxito!');
    }
    ...
    public function listar_ordenes()
    {
        ...
        $rol = $user->roles->first()->id;
        if ($rol == 1) {
            $ordenes = DB::select("SELECT o.*, c.nombre as nombre_cliente, (SELECT GROUP_CONCAT(e.ruta) FROM evidencias e WHERE e.orden_id = o.id) as evidencias FROM `ordenes` as o JOIN clientes as c ON o.id_cliente = c.NIC ORDER by o.id DESC;");
        } else {
            $ordenes = DB::select("SELECT o.*, c.nombre as nombre_cliente, (SELECT GROUP_CONCAT(e.ruta) FROM evidencias e WHERE e.orden_id = o.id) as evidencias FROM `ordenes` as o JOIN clientes as c ON o.id_cliente = c.NIC WHERE FIND_IN_SET($id, o.tecnicos_seleccionados) > 0 ORDER by o.id DESC;");
        }
